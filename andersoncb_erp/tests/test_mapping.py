...
def test_parse_entry_xml_maps_lds_datacontract_shape_and_ref_nodes():
    ...
    assert result['invoices'][0]['shipment_no'] == '7'
    assert result['invoices'][0]['invoice_amount'] == 99.25
    assert result['articles'][0]['article_line_no'] is None
    assert result['articles'][0]['invoice_number'] == '1'
    assert result['articles'][0]['shipment_no'] == '7'
    assert result['articles'][0]['line_item_identifier'] is None
    assert any(fee['fee_type'] == 'MPF' and fee['amount'] == 34.64 and not fee.get('shipment_no') for fee in result['fees'])
    assert any(fee['fee_type'] == 'HMF' and fee['amount'] == 12.34 and fee.get('shipment_no') == '7' and fee.get('invoice_number') == '1' for fee in result['fees'])
    assert any(fee['fee_type'] == 'MPF' and fee['amount'] == 5.67 and fee.get('shipment_no') == '7' and fee.get('invoice_number') == '1' for fee in result['fees'])
    assert result['articles'][0]['country_of_origin'] == 'JP'
    assert result['articles'][0]['entered_value'] is None
    assert result['tariff_lines'][0]['article_line_no'] is None
    assert result['tariff_lines'][0]['shipment_no'] == '7'
    assert result['tariff_lines'][0]['invoice_number'] == '1'
